...
*/
    ...
    public static ReviewModel[] parseResultObjectForReviewList(String parseResponse) {
        int page;
        ReviewModel[] reviewModelsList;
        Gson gson = new Gson();
        try {
            reviewModelsList = gson.fromJson(parseResponse, ReviewModel[].class);
            /*JSONArray resultArray=new JSONArray(parseResponse);
    ...
    public static TrailerMovieModel parseMovieTrailerModel(JSONObject jsonObject) {
        ...
        try {
            if (jsonObject.has(Constants.ID)) {
                trailerMovieModel.setId(jsonObject.getString(Constants.ID));
            }
